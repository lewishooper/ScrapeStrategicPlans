...
process_hospital <- function(hospital, index, total) {
  ...
  if (!isTRUE(search$pdf_downloaded) && search$pdf_downloaded != "yes") {
    ...
    if (CONFIG$verbose) {
      cat("  â””â”€ SKIP: pdf_downloaded = FALSE\n\n")
    }
  ...
  if (content_type == "html") {
    ...
    if (CONFIG$verbose) {
      cat("  â””â”€ HTML content detected... ", sep = "")
    }
    ...
    if (CONFIG$baseline_mode) {
      ...
      if (CONFIG$verbose) {
        cat("  â””â”€ BASELINE MODE: Saved as ", txt_filename, "\n\n", sep = "")
      }
  ...
  if (!validation$valid) {
    ...
    if (CONFIG$verbose) {
      cat("  â””â”€ Validation FAILED: ", validation$error_message, "\n\n", sep = "")
    }
  ...
  if (!extraction$success) {
    ...
    if (CONFIG$fallback_to_binary && !CONFIG$baseline_mode) {
      if (CONFIG$verbose) {
        cat("FAILED\n")
        cat("  â””â”€ Falling back to binary comparison... ", sep = "")
      }
  ...
  if (CONFIG$baseline_mode) {
    ...
    if (CONFIG$verbose) {
      cat("  â””â”€ BASELINE MODE: Saved as ", pdf_filename, "\n\n", sep = "")
    }
...
main <- function() {
  ...
  # =========================================================================
  # YAML UPDATE - SET EXTRACTION FLAGS BASED ON COMPARISON RESULTS
  # =========================================================================
  # Only update YAML in change detection mode (not baseline mode)
  if (!CONFIG$baseline_mode) {
    
    cat("\n")
    cat(strrep("=", 75), "\n")
    cat("UPDATING YAML WITH EXTRACTION FLAGS\n")
    cat(strrep("=", 75), "\n")
    
    # Load yaml library
    if (!require("yaml", quietly = TRUE)) {
      cat("⚠ yaml package not loaded - skipping YAML update\n")
      ...
      tryCatch({
        ...
        if (!file.exists(yaml_path)) {
          cat("⚠ YAML file not found - skipping YAML update\n")
          cat("  Expected: ", yaml_path, "\n\n")
        } else {
          ...
          # Update hospitals based on log_data
          updates <- list(
            changed_flagged = 0,
            identical_cleared = 0,
            errors = 0
          ...
          for (i in 1:nrow(log_data)) {
            ...
            if (!is.na(comparison_result) && comparison_result != "") {
              ...
              if (comparison_result == "changed") {
                ...
                # PDF unchanged - check if baseline exists
                if (isTRUE(hospital$strategy_search$baseline_extracted == "yes")) {
                  # Already extracted, no need to re-extract
          ...
          cat("  ✓ YAML updated with extraction flags\n")
        }
        
      }, error = function(e) {
        cat("  ✗ Error updating YAML: ", e$message, "\n")
      })
    }
...
save_logs <- function(log_data, summary) {
  ...
  if (CONFIG$baseline_mode) {
    ...
    cat("    - Cost Saved (unchanged):", sprintf("$%.2f", summary$api_cost_saved), 
        "(", summary$comparison_identical, " Ã— $0.08)\n")
    cat("    - Future Cost (processing):", sprintf("$%.2f", summary$api_cost_incurred),
        "(", summary$comparison_changed + summary$first_downloads, " Ã— $0.08)\n")
...
print_summary <- function(summary) {
  ...
  if (!CONFIG$baseline_mode && summary$files_saved > 0) {
    cat("\nâœ“ ", summary$files_saved, " changed/new strategic plans detected\n", sep = "")
